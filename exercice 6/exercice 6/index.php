...
if(isset($_POST['envoie'])){
    try{
        ...
        if (empty($_POST['card'])==false){
            ...
            $sql="UPDATE clients set lastName=:nom, firstName=:prenom,birthDate=:naissance,card=:card,cardNumber=:cardN where id=5";
            ...
            $sql="UPDATE clients set lastName=:nom, firstName=:prenom,birthDate=:naissance,card=:card,cardNumber=NULL where id=5";
            include('./php-pdo/connect.php');
            $val=0;
...
try{
    $check="";
    $numbC="";
    $sql="SELECT * FROM clients where id=5";
    include('./php-pdo/connect.php');
    $requete->execute();
